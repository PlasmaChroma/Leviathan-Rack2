...
Implement the complete Morphagene-style module specified by this package inside the existing Leviathan VCV Rack plugin.
...
Work through the phases in order with build/test checkpoints. Create a concise implementation-status file showing completed phases, changed files, executed commands/results, and remaining work. Do not stop permanently at a granular playback demo and call the module finished: recording topology, transport/options, sample-rate bridge, safe patch assets, cached display, and semantic control are all part of the requested implementation.
