...
Chimera attempts a pre-record checkpoint without delaying REC and, after recording stops, requests a completed cut. If a save, waveform refresh, previous checkpoint, or snapshot cleanup owns the snapshot path at REC start, recording proceeds without a new pre-record cut. The display shows **NO PRE-REC CUT** until the next take or Reel replacement. **PRE-REC SAVING** means a cut is being captured or saved; **PRE-REC FAILED** means it could not be completed. The context menu reports the latest take's pre-record status, including successful completion. An older checkpoint finishing later never marks a newer take as protected.
