...
| REC-012 | Simultaneous REC and SPLICE gates, then actual REC+SPLICE button chord. | Gate inputs remain independent; button chord invokes alternate record destination exactly once. |
...
| ID | Stimulus | Required result |
|---|---|---|
| OPT-001 | Press/release each single button and every defined two-button chord; overlap all three. | Single actions occur on release; chord executes once on first release; suppressed releases stay suppressed until all up; three-button ambiguity cancels. |
| OPT-002 | `rsop=0/1`; single REC versus REC+SPLICE. | Only default/alternate destination assignments swap; explicit semantic Current/Append commands remain explicit. |
| OPT-003 | SHIFT+REC repeated; gain targets -3,0,+6,+12 dB. | Four-state cycle, correct multipliers and 5 ms gain transition; no claim of ADC PGA simulation. |
...
| IO-009 | Save all 32 populated slots, remove/move original import directory, reload patch on clean install. | Embedded patch plays every slot without source paths; only active+one prepared mutable stores resident. |
...
| WAV-006 | Exactly 32 slot filenames and unrelated directory files. | mg1..mg9,mga..mgw mapping correct; unrelated files untouched; overwrite requires explicit permission. |
