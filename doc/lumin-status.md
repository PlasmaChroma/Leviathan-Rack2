...
## Eclipse2 hybrid-cap screening

Investigated applying HaloKnob2's raster-cap architecture to Eclipse2. A native offline probe now lives under tools/experiments/eclipse2/ with an explicit Makefile target and source-extracted reference draw methods. Eclipse2's dirty outer framebuffer bypasses the rotating cap's nested SVG cache. Four changing controls spend roughly 1.02 ms submitting that SVG geometry; retaining the cap as a texture removes most of that cost while keeping the existing ring and shadow.

Complete four-knob framebuffer rebuild/presentation medians were 1.19-1.20 ms original versus 0.14-0.17 ms hybrid across screening and reproduction runs. This is CPU timing for changing knobs, not a whole-module/FPS or idle gain; one hybrid p95 remained worse than its matched baseline. NanoSVG rasterization changed gradient shading, so the retained prototype bakes the production SVG widget through NanoVG, uses premultiplied mipmapped pixels, and registers independent main/framebuffer-context images. Even that produces resampling differences: 72 cases, maximum channel error 95/255 and maximum projected-knob mean about 2.11/255. It is not promoted to production. Visual fidelity and lifecycle/preparation integration need a live A/B path before defaults change. A ring shader is secondary to the cap bottleneck. See the experiment README for scope and reproduction; raw outputs remain under ignored build/. Native probe build/run and the unchanged plugin target were verified; no full fast-suite run was needed for this isolated experiment.

## Eclipse2 live A/B experiment

The retained-cap candidate is now wired into production Eclipse2Knob behind Integral Flux Debug > `Eclipse2 retained cap A/B (all modules)`. It starts off and is session-only/debug-only. Checking it switches all standard Eclipse2 caps to a packaged 136x136 (4x) premultiplied NanoVG bake with mipmaps; ring and shadow retain their existing renderer. The bounded shared cache owns independent images for main and framebuffer contexts and invalidates through shared lifecycle helpers. Custom SVGs, changed sizes/scales and asset/upload failures use the SVG fallback. Toggle changes invalidate stationary knob framebuffers. Draw-time upload remains included in existing module/component rendering metrics; no serialized state changes.

The native probe now calls the production candidate: packaged bake parity, debug gating, exact baseline/fallback pixels and context sharing/invalidation passed. Four changing knob framebuffer rebuild/presentation medians were 1190.7-1195.3 us baseline versus 141.3-167.9 us candidate; tails still showed occasional spikes. The prior resampling differences remain a live visual evaluation item, not a claim of pixel equivalence. See the experiment README for asset regeneration and remaining live checks.

Native Windows plugin build and all 109,950 test-fast checks passed. `make install` packaged the cap asset and copied the archive to Rack's update directory; installed/dist archive SHA256 hashes match. Rack was running during installation, so the new code takes effect after restart. No files were staged or committed.

## Eclipse2 ring optimization experiments

Tested cached LED positions, shared glow masks, a 4x static-track cache, a procedural GLSL ring, and batched shader preparation. The geometry-only path was pixel-identical but did not consistently improve complete update timing. Masks alone also showed no consistent win. Combining those with the static track reduced four changing ring rebuild/presentation medians from roughly 147-156 us to 106-125 us in the production-candidate run. Tails remain noisy. The two existing shadow passes remain unchanged (about 6.7 us for four shadows in component screening).

The procedural shader preserves round LEDs and bipolar behavior, with analytic antialiasing and bounded nearest-LED evaluation. It did not win: even batched preparation took roughly 287-311 us for four rings. It remains an offline experiment under `tools/experiments/eclipse2/`, not a live shader switch. An additional nested framebuffer costs more than this shader removes; a whole-knob/shared-atlas architecture would require a separate evaluation.

The useful cache combination is integrated as a second independent, session-only/debug-only switch: Integral Flux Debug > `Eclipse2 cached ring A/B (all modules)`. It starts off; enable alongside the retained-cap switch to combine them. The production cache uses shared NanoVG lifecycle helpers, per-context track images, the existing aperture bloom masks and fixed LED positions. Unsupported geometry and image failures retain the original renderer. Toggle changes invalidate idle framebuffers. No rendering work moves out of Draw and no patch state changes.
