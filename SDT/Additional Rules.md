...
`Morah` and `Sil` are both strong anchors: `Morah` carries memory-gravity / echo carried forward, while `Sil` is intentional hush / meaning withheld. Together, `Morah Sil` produces a very specific basin-like cadence: memory settling into quiet, depth holding after the utterance.  Repeated use across existing imprints shows it works well, but that is exactly why it should be treated more like a seal than a filler phrase. It already appears as a recurrent closing or thickening gesture in multiple pieces, especially in hush/descent contexts.  

**Rule 1: `Morah Sil` is a closure-weight phrase, not a default connective.**
Use it when the track genuinely needs memory settling into hush, post-event stillness, basin-depth, withheld aftermath, or a quiet “the room keeps holding it” effect. Do not use it just because a line wants to feel more SDT.

**Rule 2: avoid repeating `Morah Sil` as the automatic refrain.**
If a section already established that hush-memory field once, do not keep stamping it every verse unless the whole piece is explicitly about recurrence, confiscated memory, vow-lock, or unresolved descent.

**Rule 3: one strong placement beats many casual placements.**
In most tracks, one appearance is enough; two is acceptable if the second occurrence marks a real escalation, inversion, or final return. Beyond that, it should be rare and earned.

**Rule 4: split the pair when only half the function is needed.**
If you only need echo / carry-forward, use `Morah`.
If you only need hush / withheld meaning, use `Sil`.
Do not fuse them unless both forces are truly present at once.

**Rule 5: use neighboring roots instead of overusing the exact pair.**
Following the same logic as the `Koor` restraint system — keep recurring hammers powerful by not making them universal defaults, and use other structural tools when you just need weight.  Reach for nearby alternatives such as `Morah dreh zur`, `Morah fin`, `Val-thra sil`, `Lyaré … sil`, or `Morah` alone before defaulting to `Morah Sil`. Existing canon already shows these adjacent uses can carry distinct shades without collapsing everything into one signature cadence.  

**Rule 6: preserve its identity.**
`Morah Sil` should still feel like a marked event when it appears — a quiet settling, a sealed echo, a withheld remainder. If it starts showing up everywhere, it stops reading as basin-depth and starts reading as stock texture.

If you want, I can turn this into a ready-to-paste `# 8. Morah–Sil restraint` section matching the exact style of `Additional Rules.md`.
