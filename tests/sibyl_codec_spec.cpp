...
int main() {
    ...
    for (const char* doc : {R"({"schemaVersion":4})", R"({"schemaVersion":2,"composition":{"schemaVersion":3}})", R"({"patterns":{"p":{"steps":[{"step":0,"note":"C3","condition":{}}]}}})", R"({"schemaVersion":3,"harmony":{}})"})
        assert(!sibyl::parseCompositionJson(doc, 1).valid);
