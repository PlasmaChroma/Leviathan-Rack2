		...
		menu->addChild(createSubmenuItem("Sound engine", "", [m](Menu* engineMenu) {
			engineMenu->addChild(createSubmenuItem(
				"Reference V3 family", "", [m](Menu* tuningMenu) {
					...
					tuningMenu->addChild(new MenuSeparator());
					addV3Tuning(
						"Deep continuum: shorter metallic tail",
						doorstop::HelicalTuningVariant::DeepShortTail);
					addV3Tuning(
						"Deep continuum: stronger body bending",
						doorstop::HelicalTuningVariant::DeepBodyBend);
					addV3Tuning(
						"Thick spring (short tail + body bending)",
			...
			engineMenu->addChild(createCheckMenuItem(
				...
				[m]() {
					return m->engineMode.load(std::memory_order_relaxed)
						== int(doorstop::EngineMode::ReferenceV2);
				},
				[m]() {
					m->engineMode.store(
						int(doorstop::EngineMode::ReferenceV2),
