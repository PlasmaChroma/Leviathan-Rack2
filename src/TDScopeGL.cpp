  ...
  void drawFramebuffer() override {
    ...
    auto initFieldShaderPipeline = [&]() {
      ...
      static const char *kFragmentShaderSrc =
        ...
        "  float mainHotLift = 0.32 * mainHotT * mainHotT;\n"
        "  mainColor.rgb = mainColor.rgb + (vec3(1.0) - mainColor.rgb) * mainHotLift;\n"
        "  mainColor = brightenColor(mainColor, clamp(transientLift * 0.95, 0.0, 1.0));\n"
        ...
        "      float hueShift = sin(uTime * 3.0 + rowIdx * 0.1) * transientLift * 0.3 * uShdrEffect;\n"
        ...
        "  if (spanAvg <= connectorMinDelta * 1.1 || edgeDrift <= connectorMinDelta * 0.65) {\n"
        "    return;\n"
        "  }\n"
        ...
        "    float chroma = clamp(row1.w * 2.5, 0.0, 1.0) * uShdrEffect;\n"
        "    vec2 pR = p + vec2(0.0, chroma * 1.2);\n"
        "    vec2 pB = p - vec2(0.0, chroma * 1.2);\n"
        ...
        "  }\n"
        "  float vignetteFx = clamp(1.0 - length((p / vec2(uRowCount * uRowStep, uRowCount * uRowStep)) - 0.5) * 0.5, 0.0, 1.0);\n"
    ...
    auto drawLane = [&](const RowFloatAccessor &getX0, const RowFloatAccessor &getX1,
      ...
      auto drawSegmentBatch = [&](std::vector<GlSegmentQuadVertex> &verts) {
        ...
        GLsizeiptr bytes = GLsizeiptr(verts.size() * sizeof(GlSegmentQuadVertex));
        if (bytes > segmentShaderVboCapacityBytes) {
          glBufferData(GL_ARRAY_BUFFER, bytes, verts.data(), GL_STREAM_DRAW);
          segmentShaderVboCapacityBytes = bytes;
        } else {
      ...
      auto drawBatch = [&](std::vector<GlLineVertex> &verts, float width, const ShaderPassParams &shaderParams) {
        ...
        if (useGlShaderRenderer && initShaderPipeline()) {
          ...
          GLsizeiptr bytes = GLsizeiptr(verts.size() * sizeof(GlLineVertex));
