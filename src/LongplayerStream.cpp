...
std::int32_t signExtend24(std::uint32_t value) {
	return (value & 0x00800000u)
		? std::int32_t(value | 0xff000000u)
		: std::int32_t(value);
}

float decodeWavSample(
	...
	case 24:
		return float(signExtend24(readLe32(source) & 0x00ffffffu))
			* (1.f / 8388608.f);
	case 32:
...
struct Decoder {
	...
	bool openWav(const std::string& path, std::string* error) {
		...
		if (wav.gcount() != std::streamsize(sizeof(header))
		...
		if (!foundFormat || !foundData || sampleRate == 0u
			|| channels < 1u || channels > 2u || wavBlockAlign == 0u) {
	...
	bool open(const std::string& path, std::string* error) {
		...
		if (extension == ".flac") {
			...
			if (!flac || flac->channels < 1u || flac->channels > 2u
				|| flac->sampleRate == 0u || flac->totalPCMFrameCount == 0u) {
		...
		if (extension == ".mp3") {
			...
			drmp3_uint32 seekCount = drmp3_uint32(std::max<std::uint64_t>(
				32u,
				std::min<std::uint64_t>(4096u, totalFrames / std::max<std::uint64_t>(sampleRate * 2u, 1u))));
	...
	std::uint32_t readStereo(
		std::uint64_t startFrame,
		std::uint32_t framesRequested,
		float* stereoOut) {
		if (!stereoOut || startFrame >= totalFrames || !seek(startFrame)) {
			return 0u;
		}
		const std::uint32_t request = std::uint32_t(std::min<std::uint64_t>(
			framesRequested, totalFrames - startFrame));
		...
		if (kind == Kind::Wav) {
			...
			framesRead = std::uint32_t(std::max<std::streamsize>(
				0, wav.gcount()) / wavBlockAlign);
			const std::uint32_t bytesPerSample = (wavBitsPerSample + 7u) / 8u;
			for (std::uint32_t i = 0u; i < framesRead; ++i) {
				const unsigned char* source =
					wavBytes.data() + std::size_t(i) * wavBlockAlign;
				const float left = decodeWavSample(
					source, wavBitsPerSample, wavFloatingPoint);
			...
			if (kind == Kind::Flac) {
				framesRead = std::uint32_t(drflac_read_pcm_frames_f32(
					flac, request, interleaved.data()));
			}
			else if (kind == Kind::Mp3) {
				framesRead = std::uint32_t(drmp3_read_pcm_frames_f32(
					&mp3, request, interleaved.data()));
			}
			for (std::uint32_t i = 0u; i < framesRead; ++i) {
				const float left = interleaved[std::size_t(i) * channels];
				const float right = channels > 1u
					? interleaved[std::size_t(i) * channels + 1u]
					: left;
...
Stream::Stream() {
	worker = std::thread(&Stream::workerLoop, this);
}
...
void Stream::requestLoad(const std::string& pathValue) {
	{
		std::lock_guard<std::mutex> lock(requestMutex);
...
void Stream::clear() {
	requestLoad(std::string());
}

void Stream::setDesiredFrame(std::uint64_t frame, bool loop) {
	desiredFrame.store(frame, std::memory_order_relaxed);
	desiredLoop.store(loop, std::memory_order_relaxed);
}

bool Stream::readFrame(std::uint64_t frame, float* left, float* right) const {
	...
	if (frame >= start && frame - start < count) {
		...
		if (left) *left = block.stereo[index];
		if (right) *right = block.stereo[index + 1u];
		block.readers.fetch_sub(1u, std::memory_order_release);
		return true;
	}
	block.readers.fetch_sub(1u, std::memory_order_release);
...
void Stream::workerLoop() {
	...
	while (true) {
		...
		std::uint64_t serial = 0u;
		{
			...
			requestCv.wait_for(lock, std::chrono::milliseconds(5), [this]() {
		...
		if (serial != 0u) {
			...
			const bool opened = !pathToLoad.empty()
				&& decoder.open(pathToLoad, &openError);
			{
				std::lock_guard<std::mutex> lock(metadataMutex);
				loadedPath = opened ? pathToLoad : std::string();
				loadedDisplayName = opened
					? system::getFilename(pathToLoad)
					: std::string();
		...
		if (!streamReady.load(std::memory_order_acquire)
			|| decoder.totalFrames == 0u) {
			continue;
		}

		const std::uint64_t target = std::min(
			desiredFrame.load(std::memory_order_relaxed),
			decoder.totalFrames - 1u);
		const std::uint64_t base = (target / kBlockFrames) * kBlockFrames;
		std::array<std::uint64_t, kBlockCount - 2> wanted {};
		for (std::size_t i = 0; i < wanted.size(); ++i) {
			std::uint64_t start = base + std::uint64_t(i) * kBlockFrames;
			if (start >= decoder.totalFrames
				&& desiredLoop.load(std::memory_order_relaxed)) {
				start %= decoder.totalFrames;
				start = (start / kBlockFrames) * kBlockFrames;
			}
			wanted[i] = start < decoder.totalFrames
				? start
				: std::numeric_limits<std::uint64_t>::max();
		}

		bool filledOne = false;
		for (const std::uint64_t wantedStart : wanted) {
			if (wantedStart == std::numeric_limits<std::uint64_t>::max()) {
				continue;
			}
			const std::uint64_t blockNumber = wantedStart / kBlockFrames;
			Block* destination = &blocks[std::size_t(blockNumber % kBlockCount)];
			const std::uint64_t sequence =
				destination->sequence.load(std::memory_order_acquire);
			const bool present = !(sequence & 1u)
			...
			destination->validFrames = 0u;
			const std::uint32_t decoded = decoder.readStereo(
				wantedStart,
				kBlockFrames,
				destination->stereo.data());
			destination->startFrame = wantedStart;
