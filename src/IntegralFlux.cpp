...
size_t IntegralFlux::previewDebugChannelIndex(int channel) {
	return channel == 4 ? 1u : 0u;
}

void IntegralFlux::recordCurvePointReduction(int channel, size_t inputPointCount, size_t outputPointCount) {
...
void IntegralFlux::recordTracerExtraPointReduction(int channel, const WavePreviewTracerCaptureStats& stats) {
...
std::atomic<bool>& IntegralFlux::bandlimitedGateOutputsControl() {
	return bandlimitedGateOutputs;
}

std::atomic<bool>& IntegralFlux::bandlimitedSignalOutputsControl() {
	return bandlimitedSignalOutputs;
}

std::atomic<bool>& IntegralFlux::timingInterpolateControl() {
	return timingInterpolate;
}

std::atomic<bool>& IntegralFlux::previewTracerEnabledControl() {
	return previewTracerEnabled;
}

std::atomic<int>& IntegralFlux::requestedTimingUpdateDivControl() {
	return requestedTimingUpdateDiv;
}

std::atomic<int>& IntegralFlux::previewTracerCacheModeControl() {
	return previewTracerCacheMode;
}

std::atomic<int>& IntegralFlux::previewRenderModeControl() {
	return previewRenderMode;
}

uint32_t IntegralFlux::debugInstanceIdForUi() const {
...
float IntegralFlux::consumeTracerReductionAverageForUi(int channel) {
	const size_t index = previewDebugChannelIndex(channel);
...
float IntegralFlux::attenuverterGain(float knob01) {
	// Noon = 0, CCW = negative, CW = positive.
	return clamp(knob01, 0.f, 1.f) * 2.f - 1.f;
}

float IntegralFlux::injectAlphaBaseForSampleTime(float sampleTime) {
...
void IntegralFlux::enforceOuterSpeedLimit(float& riseTime, float& fallTime, float minPeriod) {
...
float IntegralFlux::shapeSignedFromKnob(float shape01, float linearShape) {
...
float IntegralFlux::shapeSignedFromKnobForMode(float shape01, FunctionShapeMode mode) {
	return shapeSignedFromKnob(shape01, mode == FUNCTION_SHAPE_SHARK_FIN ? SHARK_FIN_LINEAR_SHAPE : LINEAR_SHAPE);
}

float IntegralFlux::segmentPhaseFromOutputNorm(float outputNorm, float shapeSigned, bool rising) {
...
float IntegralFlux::segmentPhaseFromOutputNormForMode(float outputNorm, float shapeSigned, bool rising, FunctionShapeMode mode) {
	outputNorm = clamp(outputNorm, 0.f, 1.f);
...
float IntegralFlux::computeSegPhase(float out, float startOut, float invSpan) {
...
IntegralFlux::SlewStepResult IntegralFlux::processUnifiedShapedSlew(
...
float IntegralFlux::phaseCrossingFraction(float phasePos, float dp) {
...
void IntegralFlux::remapPhasePosForStageTimeChange(OuterChannelState& ch, float oldRise, float oldFall, float newRise, float newFall) {
...
void IntegralFlux::insertGateTransition(OuterChannelState& ch, bool newState, float fraction01) {
...
void IntegralFlux::setGateStateImmediate(OuterChannelState& ch, bool newState) {
	ch.gateState = newState;
}

void IntegralFlux::insertSignalTransition(OuterChannelState& ch, float step, float fraction01) {
...
void IntegralFlux::applyTimingUpdateDiv(int div) {
...
void IntegralFlux::requestTimingUpdateDiv(int div) {
	requestedTimingUpdateDiv.store(std::max(1, div), std::memory_order_relaxed);
}

void IntegralFlux::applyRequestedTimingUpdateDiv() {
...
void IntegralFlux::initKnobCurveLut() {
...
float IntegralFlux::shapeKnobTimeCurve(float knob) const {
...
void IntegralFlux::updateActiveStageTimes(OuterChannelState& ch) {
...
void IntegralFlux::publishPreviewState(PreviewSharedState& shared, float riseTime, float fallTime, float curveSigned,
...
void IntegralFlux::publishPreviewDot(PreviewSharedState& shared, bool visible, float xNorm, float yNorm) {
...
bool IntegralFlux::previewChangedMeaningfully(float riseNow, float risePrev, float fallNow, float fallPrev, float curveNow, float curvePrev) {
...
bool IntegralFlux::updatePreviewChannel(
...
void IntegralFlux::getPreviewState(int channel, float& riseTime, float& fallTime, float& curveSigned, float& dotXNorm,
...
float IntegralFlux::computeShapeTimeScale(float shape, FunctionShapeMode mode, float logScaleLog2, float expScaleLog2) const {
...
float IntegralFlux::computeStageTime(
	float knob,
	float stageCv,
	float bothScale,
...
void IntegralFlux::triggerOuterFunction(OuterChannelState& ch) {
	// Trigger always starts a fresh rise phase.
	ch.phase = OUTER_RISE;
	ch.phasePos = 0.f;
}

IntegralFlux::OuterChannelResult IntegralFlux::processOuterChannel(
...
json_t* IntegralFlux::dataToJson() {
...
void IntegralFlux::dataFromJson(json_t* rootJ) {
